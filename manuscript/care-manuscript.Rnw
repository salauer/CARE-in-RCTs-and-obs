...
est_sim <- readRDS(find_recent_file(name_start="sim-output",
                                path="CARE-paper/data/simulation-output/")) %>%
    mutate(name=ifelse(name=="IPTW","IPW",as.character(name)))

evaluation_sim <- est_sim %>%
    group_by(trial, PATE, y_formula, a_formula, name) %>%
    summarise(ave = mean(pt),
              bias = mean(pt - PATE),
              abs_bias = abs(mean(pt - PATE)),
              mc_var = var(pt),
              mc_se=sqrt(mc_var),
              est_var = mean(se^2),
              est_se=mean(se),
              ...
              mse = mean((pt - PATE)^2),
              mae = mean(abs(pt - PATE)),
              coverage = mean(cover),
              pwr=mean(reject[PATE!=0]),
              type1_err = mean(reject[PATE==0])) %>%
    ungroup() %>%
    mutate(y_formula=ifelse(name %in% c("unadj", "IPTW"),
                            "none", as.character(y_formula)) %>% as.factor,
           a_formula=ifelse(name %in% c("unadj", "CARE"),
                            "none", as.character(a_formula)) %>% as.factor,
           PATE=factor(PATE, labels=c("Effect", "Null")),
...
\and
\author{\fnms{Laura B.} \snm{Balzer}\thanksref{t2}\ead[label=e3]{lbalzer@umass.edu}}
...
\kwd{inverse probability treatment weighting}
\kwd{confounding}
\kwd{randomized trials}
\kwd{observational studies}
\end{keyword}

\end{frontmatter}

Estimating the effect of an exposure on a population has a long history.
In 1855, John Snow compared the mortality rates of households in London by the company that supplied their water to locate the source of a cholera epidemic.\citep{Snow1855}
In 1881, Louis Pasteur inoculated 50 sheep with anthrax, 25 of whom had been vaccinated; the vaccinated sheep survived as the unvaccinated died, providing evidence that his anthrax vaccine was effective.\citep{Merrill2010}
In 1948, Austin Bradford Hill conducted the first modern randomized clinical trial, to evaluate a treatment for pulmonary tuberculosis,\citep{Hill1948, Yoshioka1998} and later formulated guidelines for researchers and practitioners to transition from thinking in terms of statistical association to those of causation.\citep{Hill1965}
Since then, there has been a proliferation of methods to determine the exposure effects in randomized trials and observational studies.\citep{Fisher1932, Horvitz1952, Cochran1957, Cox1982, Rosenbaum1983, Robins1986, VanderLaan2006, Tsiatis2008, Hayes2009, Moore2009, Rosenblum2010, Rose2011, Balzer2016}

One such method is the covariate-adjusted residuals estimator (CARE), which was formulated to estimate the effect of an exposure on an outcome of interest in individually randomized or cluster randomized trials.\citep{Gail1996, Bennett2002, Hayes2009}
...
Most recently, CARE was used to investigate the effect of a Universal Test-and-Treat intervention on HIV incidence in a large cluster randomized trial in Zambia and South Africa.\citep{hayes_impact_2019}
To the best of our knowledge, the properties of CARE in a randomized trial have not been evaluated non-parametrically.

In observational settings, CARE is commonly used in ecology under the name `residual index',\citep{Jakob1996, Kotiaho2001, Blackwell2002, ELKIN2005, Sanchez-Chardi2007, Danielson-Francois2009} although it has received some criticism.\citep{Green2001,Garcia-Berthou2001}
...
While there are domain-specific questions about whether ordinary least squares linear regression is being used appropriately in allometry,\citep{Green2001} others have questioned the statistical validity of the residual index under any circumstances.
...
We compare CARE to existing estimators and introduce a novel estimator for use in both randomized and observational settings; CARE-IPW, our new estimator, combines CARE with methods using inverse probability weighting.\citep{Horvitz1952, Robins2000, Rosenbaum1983} We prove CARE-IPW is an unbiased estimator in both randomized trials and observational settings, and provide the conditions for consistency and asymptotic normality.
...
Both the CARE rate ratio (RR: 0.844, 95\% CI: 0.713, 0.999) and rate difference (RD: -4.26, 95\% CI: -8.76, 0.23) estimates indicated a stronger association than that found by the unadjusted estimator (RR: 0.859, 95\% CI: 0.721, 1.023; RD: -3.95, 95\% CI: none provided).\citep{Hayes2009}
...
Several conceptual and analytic frameworks exist and can guide our answering of causally motivated questions.\citep{Neyman1923, Rubin1974, Robins1986, Rubin1990, Holland1986, Spirtes1993, Pearl2009, Robins2009, Richardson2013swig,Petersen2014, balzer_tutorial_2016,  Hernan2016}.
Here, we review the Causal Roadmap of \cite{Petersen2014} and use the bednet example for illustration.
The key steps of the Causal Roadmap are representing knowledge of the data generating process (represented by a \textit{causal model}); specifying the quantity that answers the scientific question (the \textit{causal parameter}); evaluating the assumptions required to link the causal quantity to a well-defined function of the observed data distribution (the \textit{statistical parameter} which may or may not be \textit{identifiable}); and finally obtaining a point estimate and inference of the statistical parameter.%\citep{Pearl1988, Pearl2009, Pearl2010, Petersen2014}

\begin{figure}
\includegraphics[width=0.49\textwidth]{figure/rct-diagram.jpeg}
\includegraphics[width=0.49\textwidth]{figure/obs-diagram.jpeg}
\caption{
...
A causal model is a structural framework for expressing the relationships between variables in a given setting.\citep{Pearl1988, Pearl2009, Pearl2010, Goldberger1972, Duncan1975}
...
We assume that the relationships within the causal model are autonomous, meaning that changing one relationship does not change the other relationships, although changes may result in different effects downstream.\citep{Pearl2009,Pearl2010}
Therefore, we could modify the way in which the exposure is generated and see resulting changes in the outcome.
Specifically, we could intervene to give impregnated bednets to a cluster (i.e. set $A=1$) %, leaving the other relationships the same,
and generate the counterfactual (potential) outcome $Y(1)$ for that cluster.
Likewise, we could intervene to put the same cluster in the unexposed group (i.e. set $A=0)$ %, leaving the other relationships the same,
...
\begin{align}
    ATE = \mathbb{E}[Y(1)-Y(0)]. \label{eqn:pate}
\end{align}
...
\end{align}
This statistical parameter $\Psi^{obs}(\mathbb{P})$ is known as the ``G-computation identifiability result'', which identifies the ATE under two assumptions.\citep{Robins1986}
First, there must be no unmeasured confounding between the exposure and the outcome: $Y(a) \independent A \mid W^C$.
Second, the `positivity assumption', which states that each possible strata of measured confounding covariates has a non-zero probability of being in each exposure group ($\mathbb{P}(A=a | W^C=w^C)>0, \forall w^C \in \mathbb{P}(W^C=w^C)>0$), must hold.\citep{Petersen2010}
...
\end{align}
The statistical parameter $\Psi^{RCT}(\mathbb{P})$ can be consistently estimated using the difference in the average outcome between the exposure groups, also known as the `unadjusted estimator':\citep{Neyman1923}
\begin{align}
...
\begin{figure}[hbt!]
\includegraphics[width=.7\linewidth]{figure/big-app-fig-1.pdf}
\caption{
...
If $W^C$ captures all those common causes (i.e. there is no unmeasured confounding) and there is sufficient variability in the exposure within possible strata of $W^C$ (i.e. positivity holds), then the inverse probability weighting (IPW) estimator can be used to estimate the statistical parameter $\Psi^{obs}(\mathbb{P})$:\citep{Horvitz1952,Robins2000}
\begin{align}
	\hat{\Psi}^{IPW}(\hat{\mathbb{P}}) &= \frac{1}{n} \sum_{i=1}^n \left(\frac{\mathbb{I}(A_i=1)}{\hat{\mathbb{P}}(A=1 \mid W^C_i)} - \frac{\mathbb{I}(A_i=0)}{1-\hat{\mathbb{P}}(A=1 \mid W^C_i)} \right) Y_i, \label{eqn:iptw}
\end{align}
which controls for the confounding covariates through estimates of the conditional probability of receiving the exposure, called `propensity scores' $\hat{\mathbb{P}}(A=1 \mid W^C)$.\citep{Rosenbaum1983}
...
IPW can also be used in randomized trials, and in that setting estimating the known propensity score can lead to efficiency gains over the unadjusted estimator.\citep{vanderLaan2003, Shen2014, Balzer2016}
...
% \begin{figure}[hbt!]
% \includegraphics[width=0.49\textwidth]{figure/rct-unadj.pdf}
% \includegraphics[width=0.49\textwidth]{figure/rct-care.pdf}
% \includegraphics[width=0.49\textwidth]{figure/obs-iptw.pdf}
% \includegraphics[width=0.49\textwidth]{figure/obs-care-ipw.pdf}
% \caption{
% Diagrams that indicate which covariates are used by each estimator.
% \textbf{(a)} The unadjusted estimator does not use any covariates and only compares the average outcome $Y$ between exposure levels $A=0$ and $A=1$.
% The unadjusted estimator is consistent for the target statistical parameter in randomized settings $\Psi^{RCT}(\mathbb{P})$ (shown), but not observational settings.
% \textbf{(b)} The covariate-adjusted residuals estimator (CARE) incorporates baseline covariates to make predictions for the outcome $\hat{\mathbb{E}}(Y \mid W^Y=w^Y)$.
% If these baseline covariates are predictive of the outcome and imbalanced between exposure levels, then CARE should be more efficient than the unadjusted estimator in randomized settings.
% CARE is not consistent for $\Psi$ in observational settings with an exposure effect.
% \textbf{(c)} The inverse probability weighting (IPW) estimator is consistent for $\Psi$ when its propensity scores $\hat{\mathbb{P}}(A=1 \mid W^C=w^C)$ are consistently estimated in observational settings.
% \textbf{(d)} When CARE is augmented by inverse probability weighting (CARE--IPW), it is consistent for $\Psi$ when its propensity scores $\hat{\mathbb{P}}(A=1 \mid W^C=w^C)$ are consistently estimated.
% CARE--IPW may be more efficient than the IPW estimator when accounting for $W^C$ in its predictions of the outcome $\hat{\mathbb{E}}(Y \mid W^C=w^C)$.
% }
% \label{fig:dag-est}
% \end{figure}
...
The covariate-adjusted residuals estimator (CARE) was proposed as a method to estimate the ATE in randomized trials.\citep{Gail1996, Bennett2002, Hayes2009}
...
%In practice, this prediction has relied on parametric regression models {\color{red}[LB: Steve; please confirm]}.
...
\begin{thm}
In a randomized trial, the covariate adjusted residuals estimator (CARE) is an unbiased estimator of the target statistical parameter $\Psi^{RCT}(\mathbb{P}) = \mathbb{E}(Y|A=1) - \mathbb{E}(Y|A=0)$ and thus the average treatment effect (ATE) because the identifiability assumptions hold by design.
\end{thm}
The proof is given in Appendix \ref{app:care-rct}. Briefly, consider the following estimating function of the observed data $O=(W^Y,A,Y)$ and parameter $\psi$: \citep{Rose2011, Kennedy2017}
\begin{align}
...
Under regularity conditions,\citep{Rose2011, Kennedy2017} the Central Limit Theorem applies, and CARE is asymptotically normal with variance well-approximated by the sample variance of $\hat{D}(O;\hat{\psi}^{CARE})$ divided by sample size $n$.
...
When accounting for predictive covariates $W^Y$ that are imbalanced by chance between the two randomized groups, we expect CARE to provide efficiency gains over the unadjusted estimator. \citep{Fisher1932, Cochran1957, Cox1982, Tsiatis2008, Moore2009, Rosenblum2010, Balzer2016} When the predicted values of the outcome $\hat{\mathbb{E}}(Y \mid W^Y)$ are a constant value (e.g. zero or the mean of all observations $\hat{\mathbb{E}}(Y)$), then CARE is equivalent to the unadjusted estimator (proof in Appendix \ref{app:care-rct}).
Thus, the unadjusted estimator could be considered a special case of CARE.


\begin{thm}
In most observational settings of interest, the covariate adjusted residuals estimator (CARE) is a biased estimator of the target statistical parameter $\Psi^{obs}(\mathbb{P})=\mathbb{E}_{W^C}\big[ \mathbb{E}(Y|A=1, W^C) - \mathbb{E}(Y|A=0,W^C) \big]$.
\end{thm}
...
%Thus, we conclude that CARE is generally \textbf{not} an unbiased for $\psi^{obs}$, even if the covariates $W^C$, which are sufficient to control for confounding, are used to predict the outcome.
...
\begin{thm}
In an observational setting, the covariate-adjusted residuals estimator with inverse probability weighting (CARE--IPW) is an unbiased estimator of the target statistical parameter $\Psi^{obs}(\mathbb{P}) = \mathbb{E}_{W^C}\big[ \mathbb{E}(Y|A=1, W^C) - \mathbb{E}(Y|A=0,W^C) \big]$.
...
Under regularity conditions,\citep{Rose2011, Kennedy2017} the Central Limit Theorem applies, and CARE--IPW is asymptotically normal with variance well-approximated by the sample variance of $\hat{D}^*(O;\hat{\psi}^{CARE-IPW})$ divided by sample size $n$.
By predicting the outcome with the covariates $\hat{\mathbb{E}}(Y \mid W^C)$, we expect CARE-IPW to provide efficiency gains over the IPW estimator.
 %{\color{red}[LB: think this is only the case, if we use different adjustment sets for $E(Y|W^C)$ and $P(A|W^C)$]}
%{\color{purple} [SL: if the adjustment sets for $E(Y|W^C)$ and $P(A|W^C)$ have to be different but $\mathbb{P}(A=1 \mid W^C)$ has to be consistently estimated for CARE--IPW to be consistent, then doesn't that mean that to get efficiency gains we would lose consistency? Maybe the adjustment set should be restricted to $E(Y|W^Y)$]}
...
All simulations were run using R version 3.4.3.\citep{R2018}
...
When there is an effect, the intervention $A$ led to a \Sexpr{formatC(PATE_effect*100,digits=1,format="f")}\% average reduction in the outcome.
...
unadj_obs_effect_ci <- formatC(100*evaluation_sim$coverage[evaluation_sim$trial=="Obs" & evaluation_sim$PATE=="Effect" & evaluation_sim$name=="unadj"],1, format="f")
care_obs_effect_ci <- formatC(100*evaluation_sim$coverage[evaluation_sim$trial=="Obs" & evaluation_sim$PATE=="Effect" & evaluation_sim$name=="CARE"],1, format="f")
ipw_obs_effect_power <- formatC(100*evaluation_sim$pwr[evaluation_sim$trial=="Obs" & evaluation_sim$PATE=="Effect" & evaluation_sim$name=="IPW"],1, format="f")
cipw_obs_effect_power <- formatC(100*evaluation_sim$pwr[evaluation_sim$trial=="Obs" & evaluation_sim$PATE=="Effect" & evaluation_sim$name=="CARE_IPW"],1, format="f")
unadj_obs_null_ci <- formatC(100*evaluation_sim$coverage[evaluation_sim$trial=="Obs" & evaluation_sim$PATE=="Null" & evaluation_sim$name=="unadj"],1, format="f")
...
In this section, we first reproduce the findings of \cite{Hayes2009}, who compared CARE to the unadjusted estimator in the cluster randomized trial to estimate the impact of impregnated bednets on child mortality in northern Ghana.\citep{Bennett2002, Binka1996}
...
%These covariates may be relevant as younger children are more vulnerable than older children, and young males typically have higher mortality rates than young females. {\color{red}[cite]}
For CARE--IPW, we use the same predicted values of the outcome from the individual-level regression as used for CARE.
%We estimate the average treatment effect and variance for CARE--IPW and the IPW estimator using the methods outlined in Section \ref{sec:care-ipw}.

% All estimates are for the target statistical parameter $\Psi$.
% The positivity and randomization assumptions hold by the design of the cluster randomized trial, however it is unclear whether the assumption of no interference between units is satisfied.
% Therefore, in order for $\Psi$ to identify the average treatment effect, we need to assume that the childhood mortality in each cluster is independent of the assignment to bednets and childhood mortality in the other clusters.
...
%The unadjusted estimator, the inverse probability weighting (IPW) estimator, and CARE can all be considered special cases of CARE--IPW.
%In a randomized trial, we would expect CARE to be more efficient than the unadjusted estimator when the predictions for the outcome account for predictive covariates that are imbalanced between exposure levels.
%{\color{red}[LB - what do we expect about CARE-IPW]}
...
While CARE--IPW improves on CARE and IPW, it is not a ``double robust estimator'', such as targeted maximum likelihood estimation and augmented inverse probability weighting.\citep{%Scharfstein1999,
robins2000robust, vanderLaan2003, VanderLaan2006, MarkBook}
...
By incorporating predictions of the outcome, CARE--IPW is expected to be more efficient than IPW.%improves efficiency over the IPW estimator by making predictions of the outcome.
...
% {\color{red}[LB: although one way around this (and often recommend in RCT) is to fully stratify on the exposure]}

As with the IPW estimator, CARE--IPW may have stability issues when estimated propensity scores approach zero or one.\citep{Petersen2010}
This could be resolved in one of a couple ways.
Stabilized weights could be used to scale propensity scores away from zero and one.\citep{Robins2000}
Alternatively, propensity scores could be replaced by incremental propensity scores which relax the positivity assumption by looking at the effect of an intervention when propensity scores are uniformly increased and decreased across all observations.\citep{Kennedy2018}
...
As an alternative to CARE, CARE--IPW has potential for use as an estimator in observational settings; further research is warranted.%, though more research is required to investigate its performance in different situations relative to other estimators.
...
\mathbb{E} &\left[ \left( \frac{\mathbb{I}(A=1)}{\mathbb{P}(A=1 \mid W^C)} - \frac{\mathbb{I}(A=0)}{\mathbb{P}(A=0 \mid W^C)}\right) Y \right] \\
	...
	&= \psi^{obs}
\end{align*}
%where summations generalize to integrals for continuous random variables here and throughout.
...
\begin{supplement}
\label{supmat}
\sname{Supplemental materials}
\slink[url]{http://www.filler.link/supplemental-materials}
\sdescription{DAGs for randomized and observational settings with unobserved covariates.}
\end{supplement}
