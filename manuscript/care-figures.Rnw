...
## read in bednet data
ghana_bednet_data <- read_table2("CARE-paper/data/dataverse_files/ghana_bednet.tab")
...
<<application-plot, echo=FALSE, warning=FALSE, message=FALSE, fig.asp=0.5, out.width='100%', fig.cap="The estimates and 95\\% confidence intervals for the difference in childhood mortality rate per thousand person-years associated with the allocation of bednets. The four estimators used are the unadjusted estimator, the covariate-adjusted residuals estimator (CARE), the inverse-probability of treatment weighting estimator (IPW), and CARE with inverse-probability weighting (CARE--IPW). All estimates indicate that bednets caused a reduction in childhood mortality, though the CARE and CARE--IPW estimates are more precise than those of the unadjusted and IPW estimators.">>=
...
plot_dat <- tibble(name=c("Unadjusted",
                   ...
                   pval=round(c(p_unadj,
                                ...
                                p_CARE,
                                p_CARE_IPW),2))
write.csv(plot_dat, "CARE-paper/data/app-output.csv")
...
# app_care <- ggplot(gb_care, aes(x=agemn, y=Y-Yhat)) +
#   geom_point(aes(color=as.factor(bednet))) +
#   geom_hline(aes(yintercept=Y0bar_CARE), color=cbbPalette[2]) +
#   geom_hline(aes(yintercept=Y1bar_CARE), color=cbbPalette[3]) +
#   geom_hline(aes(yintercept=0), color=cbbPalette[4], linetype="dashed") +
#   geom_linerange(aes(ymin=Y1bar_CARE, ymax=Y0bar_CARE, x=27.75), linetype="dotted") +
#   scale_x_continuous(expression(paste("Average cluster age, in months (", italic("W"^"Y"), ")"))) +
#   scale_y_continuous(expression(paste("Child mortality rate residual, per 1,000 follow-up years  (", italic(paste("Y-", hat("Y"))), ")")),
#                      breaks=seq(-50,50,10)) +
#   scale_color_manual("",
#                      values=cbbPalette[c(2,3)],
#                      labels=c(expression(paste("No bednets (", italic("A=0"),")")),
#                               expression(paste("Bednets (", italic("A=1"),")")))) +
#   ggtitle("Covariate-adjusted residuals estimate") +
#   theme_bw()+
#   theme(axis.text=element_text(size=12, color="black"),
#         axis.title=element_text(size=14),
#         legend.text=element_text(size=12),
...
# app_care_ipw <- ggplot(gb_care, aes(x=agemn, y=Y-Yhat)) +
#   geom_point(aes(color=as.factor(bednet), size=weight)) +
#   geom_hline(aes(yintercept=Y0bar_CARE_IPW), color=cbbPalette[2]) +
#   geom_hline(aes(yintercept=Y1bar_CARE_IPW), color=cbbPalette[3]) +
#   geom_hline(aes(yintercept=0), color=cbbPalette[4], linetype="dashed") +
#   geom_linerange(aes(ymin=Y1bar_CARE_IPW, ymax=Y0bar_CARE_IPW, x=27.75), linetype="dotted") +
#   scale_x_continuous(expression(paste("Average cluster age, in months (", italic("W"^"Y"), ")"))) +
#   scale_color_manual("",
#                      values=cbbPalette[c(2,3)],
#                      labels=c(expression(paste("No bednets (", italic("A=0"),")")),
...
<<ic-curves, echo=FALSE, warning=FALSE, message=FALSE, fig.asp=1, out.width='100%', fig.cap="The estimates and 95\\% confidence intervals for the difference in childhood mortality rate per thousand person-years associated with the allocation of bednets. The four estimators used are the unadjusted estimator, the covariate-adjusted residuals estimator (CARE), the inverse-probability of treatment weighting estimator (IPW), and CARE with inverse-probability weighting (CARE--IPW). All estimates indicate that bednets caused a reduction in childhood mortality, though the CARE and CARE--IPW estimates are more precise than those of the unadjusted and IPW estimators.">>=
